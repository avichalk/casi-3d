...
def co_preprocessing(data_path='../data/CleanCO'):
    ...
    if os.path.isfile('../data/temp_co/binary_co_log_noise_0825.npy'):
        x,y=np.load('../data/temp_co/binary_co_log_noise_0825.npy')
    ...
    #    y = pad_data(y)
        ...
        x = np.expand_dims(x, axis=-1)
        y = np.expand_dims(y, axis=-1)
        np.save('../data/temp_co/binary_co_log_noise_0825.npy',[x,y])
