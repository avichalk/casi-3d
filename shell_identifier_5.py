...
def main():
    ...
    with open('hypers_5.json', 'r') as f:
        params = json.load(f)
    ...
    hypers["error"] = error
    log_hypers('../models/hypers_5.csv', hypers)
    ...
    def new_init(self, model_hypers):
        if self.gpu_count > 1:
            ...
            self.multi_gpu_model = multi_gpu_model(self.model,
                                                   gpus=self.gpu_count)
            self.multi_gpu_model.compile(optimizer=SGD(lr=0.01, momentum=0.9), loss='mse')
        else:
            self.model = self.build_model(**model_hypers)
            self.model.compile(optimizer=SGD(lr=0.01, momentum=0.9), loss='mse')
